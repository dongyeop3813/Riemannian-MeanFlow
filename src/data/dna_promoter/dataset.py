...
class MemmapGenome(Genome):
    ...
    def get_encoding_from_coords(self, chrom, start, end, strand="+", pad=False):
        ...
        #assert encoding.shape[1] == end - start, (encoding.shape, end - start)
        return encoding.T
    ...
    def sample(self, batch_size=1, mode=None, coordinate_only=False):
        ...
        for i, (chroms, starts, ends, strands) in enumerate(allcoords):
            if not coordinate_only:
                ...
                if self.bg:
                    if self.target_1d:
                        seq, seq_targets, seq_background, seq_target_1ds = (
                            retrieve_output
                        )
                    else:
...
class PromoterDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        data_dir: str = "data/dna_promoter",
        seqlength=1024,
        split="train",
        n_tsses=100000,
        rand_offset=0,
        ...
        config = argparse.Namespace(
            seifeatures_file=str(data_dir / "target.sei.names"),
            seimodel_file=str(data_dir / "best.sei.model.pth.tar"),
            ref_file=str(data_dir / "Homo_sapiens.GRCh38.dna.primary_assembly.fa"),
            ref_file_mmap=str(
                data_dir / "Homo_sapiens.GRCh38.dna.primary_assembly.fa.mmap"
            ),
            tsses_file=str(
                data_dir / "FANTOM_CAT.lv3_robust.tss.sortedby_fantomcage.hg38.v4.tsv"
            ),
            fantom_files=[
                str(data_dir / "agg.plus.bw.bedgraph.bw"),
                str(data_dir / "agg.minus.bw.bedgraph.bw"),
            ],
            fantom_blacklist_files=[
                str(data_dir / "fantom.blacklist8.plus.bed.gz"),
                str(data_dir / "fantom.blacklist8.minus.bed.gz"),
            ],
            n_time_steps=400,
            random_order=False,
            speed_balanced=True,
...
if __name__ == "__main__":
    # Save test data as numpy array
    dataset = PromoterDataset(
        "/network/scratch/m/marta.skreta/meanflows/data/dna_promoter", rand_offset=0, split="valid", sep_x_y=True
    )
    print(dataset[0])
