...
from .ff_energy import compute_mmff_energy
...
class SampleAnalyzer:
    ...
    def analyze(
        self, sampled_molecules: List[SampledMolecule], return_counts: bool = False
    ):
        ...
        validity_result = self.compute_validity(
            sampled_molecules, return_counts=return_counts
        )
        if return_counts:
            (
                frac_valid_mols,
    ...
    def compute_energy_divergence(self, samples: List[SampledMolecule]):
        ...
        if self.processed_data_dir is None:
            raise ValueError(
                "You must specify processed_data_dir upon initialization to compute energy divergences"
            )
...
def check_stability(molecule: SampledMolecule):
    ...
    for i, (atom_type, valency, charge) in enumerate(
        zip(atom_types, valencies, molecule.atom_charges)
    ):
        valency = int(valency)
        charge = int(charge)
...
def make_plot(sampled_molecules: List[SampledMolecule]):
    """
    Make a plot of the sampled molecules using RDKit 3D visualization.

    Args:
        sampled_molecules: List of SampledMolecule objects to visualize
    """
    ...
    n_mols = min(len(sampled_molecules), 4)
    molecules_to_plot = sampled_molecules[:n_mols]

    view = py3Dmol.view(width=420, height=300, viewergrid=(2, 2))

    for i, mol_obj in enumerate(molecules_to_plot):
        mol = mol_obj.rdkit_mol
        mb = Chem.MolToMolBlock(mol)
        view.addModel(mb, "mol", viewer=(i // 2, i % 2))
        view.setStyle({"stick": {}})
        view.zoomTo()
